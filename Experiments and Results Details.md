...
Table S4: Hyperparameter search ranges
...
|<p>**Modality**</p><p>**(Model Type)**</p>|**Convolution Layers**|**Inception block**|**Pooling layer**|
| :- | :- | :- | :- |
...
|source fusion (C)|<p>{16, 32, 64, 128}</p><p></p>|<p>{3, 5, 7}</p><p></p>|<p>{3, 5, 7}</p><p></p>|<p>[4, 32]</p><p></p>|<p>{3, 5}</p><p></p>|{Max, Average}|{0.3, 0.4, 0.5, 0.6, 0.7}|{Max, Average}|
...
|**Modality**|**Audio Pool**|**Video Pool**|**1D convolution**|**Inception block**|**Pooling layer**|
| :- | :- | :- | :- | :- | :- |
||**Pooling type**|**Pooling type**|**# filters\***|**Common kernel size (k)**|**# filters**|**Pool size**|**Pooling type**|**Dropout rate**|**Pooling type**|
|latent fusion (D)|{Max, Average}|{Max, Average}|{16, 32, 64, 128}|{3, 5, 7}|<p>[4, 32]</p><p></p>|<p>{3, 5}</p><p></p>|<p>{Max, Average}</p><p></p>|<p>{0.3, 0.4, 0.5, 0.6, 0.7}</p><p></p>|{Max, Average}|
...
||Random Forest|Num estimators|10,25,50,75,100|
|||Max depth|3,5,7|
...
|||max\_depth|2,4,6|
|||Min\_child\_weight|9,11|
|||subsample|0.7,0.8|
|||colsample\_bytree|0.7,0.5,0.6|
|||n\_estimators|10-100 in steps of 10|
...
Tables S5: Tuned hyperparameters
...
Table S5A1: Tuned hyperparameters for video classification task on seen singer split
...
Table S5A2: Tuned hyperparameters for video classification task on unseen singer split
...
Table S5B1: Tuned hyperparameters for audio classification task on seen singer split
...
Table S5B2: Tuned hyperparameters for audio classification task on unseen singer split
...
Table S5C: Tuned hyperparameters for early fusion classification
...
Table S5D: Tuned hyperparameters for latent fusion classification
...
Table S5E: Tuned hyperparameters for Late fusion

|Split|Model|Hyperparameter name|
| :- | :- | :- |
|||Num\_Estimators|Max\_depth|Max\_features|
...
Figure S3: Confusion matrices of the results
...
![](Aspose.Words.7a834eb5-c595-48ae-9c67-c7a7585c94bc.002.png)Figure S3 (b): Confusion matrices of predictions made from audio, video and audio-video modalities. Numbers are represented in percentages of the total number of test examples across the three singers combined. Same as Figure 5 in the paper.
|||1 of 2|
| :- | :-: | -: |
